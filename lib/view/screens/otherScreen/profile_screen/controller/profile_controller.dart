...
class ProfileController extends GetxController {
  ...
  void togglePushNotification(bool value) {
    ...
    // You can also call API here to save the preference
...
//========== getUserProfile controller ==========
  ...
  // ========= Help  and Support =================
  Rx<TextEditingController> titleHelpController = TextEditingController().obs;
  Rx<TextEditingController> desHelpController = TextEditingController().obs;
  RxBool helpLoading = false.obs;

  Future<void> postHelpAndSupport() async {
    helpLoading.value = true;

    Map<String, dynamic> body = {
      "title": titleHelpController.value.text.trim(),
      "description": desHelpController.value.text.trim().toLowerCase(),
    };
    try {
      var response = await ApiClient.postData(ApiUrl.helpSupport, jsonEncode(body));
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        showCustomSnackBar(jsonResponse['message']?.toString() ?? "Submit successful!", isError: false);
  ...
  ///========== Clear SignUp Data ==========
  void clearHelpData() {
    titleHelpController.value.clear();
    desHelpController.value.clear();
  }
